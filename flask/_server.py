...
def update_line_crossing(cam_name, tracks, img_w, now_ts):
	with thread_lock:
		count = counter.setdefault(cam_name, {'L2R':0, 'R2L':0})
	...
	for track in tracks:
		...
		if (x_prev < line) and (cx_norm > line):
			crossed = 'L2R'   # ซ้ายไปขวา
		elif (x_prev > line) and (cx_norm < line):
			crossed = 'R2L'   # ขวาไปซ้าย
		
		if crossed:
			ts_prev = ts_prevs.get(tid, 0.0)
			if (now_ts - ts_prev) >= CROSS_COOLDOWN: # ป้องกันนับซ้ำ
				count[crossed] += 1
				ts_prevs[tid] = now_ts
...
def yolo_deepsort_process(cam_name, frameB64):
	...
	for box in r0.boxes:
		...
		if int(box.cls) == 0: # ตรวจจับเฉพาะคน (class 0)
			# เพิ่ม [bounding box], confidence ลงในรายการ detections
			detections.append([[x1, y1, x2, y2], confidence])

	# ติดตามวัตถุด้วย DeepSort
	with thread_lock:
		if cam_name not in camera_tracker:
			camera_tracker.setdefault(cam_name, make_tracker())
		# update_tracks() ต้องการ frame เป็น BGR เสมอ
		# และรับ detections เป็น [[x1,y1,x2,y2], confidence]
		# คืนค่าเป็นรายการ track objects
		tracks = camera_tracker[cam_name].update_tracks(detections, frame=frame_bgr)

	# นับจำนวน track ที่ข้ามเส้น
	h, w = frame_bgr.shape[:2] # ดึงขนาดภาพ, [:2] คือการตัดเอา list 2 ค่าแรก (สูง, กว้าง)
	now = time.time()
	update_line_crossing(cam_name, tracks, w, now)

	# วาดกรอบ, เส้น, และ id ลงบนภาพ
	out = frame_bgr.copy()
	...
	while True:
		...
		with thread_lock:
			for cam_name, dq in camera_frame.items():
				if dq:
					jobs.append((cam_name, dq[-1])) # ใช้ frame ล่าสุดใน deque
