...
def update_line_crossing(cam_name, tracks, img_w, now_ts):
	...
	for track in tracks:
		...
		if crossed:
			ts_prev = ts_prevs.get(tid,0)
			if now_ts - ts_prev >= CROSS_COOLDOWN:
				counter[cam_name][crossed] += 1
				last_cross_ts[cam_name][tid] = now_ts
...
def yolo_deepsort_process(cam_name, frameB64):
	...
	with thread_lock: #ป้องกันการเเย่งใช้ตัวแปร
		if cam_name not in camera_tracker: #กล้องนี้มีการใช้ตัว track รึยัง
			camera_tracker[cam_name] =make_tracker()
		track = camera_tracker[cam_name].update_tracks(detections, frame=frame_bgr)
